...
Merged to `main` (fast-forward, `a59c890`) and its worktree/branch cleaned
up same day.

Re-run `/improve` against this repo in the future to catch anything new
that's landed since this pass.

## Sixteenth pass — 2026-09-04

Single targeted plan (`plan <description>` mode, no full audit). The
owner hit plan 045's auto-pay revert guard mid-troubleshooting — toggling
a bill's Auto Pay checkbox while testing, lost track of its saved state,
saw a grayed-out "Revert to Pending" icon with no visible explanation
beyond a hover-only tooltip, and got stuck genuinely confused ("I cannot
fix the USI: Internet bill... or am I missing something?"). Live-DB check
confirmed the guard was working exactly as designed (`isAutoPay: true`)
— not a bug, a discoverability gap. Plan 048 removes the client-side
disabled gate entirely and lets every click attempt the revert — the
server-side guard and its error-toast plumbing (both already built by
plan 045) already surface the exact right explanation the moment someone
actually tries, which turned out to need no new code, just an unblocked
path to what already existed.
...
| Plan | Title | Priority | Effort | Risk | Depends on | Status |
|------|-------|----------|--------|------|------------|--------|
| 049  | Fix 6 known vulnerabilities in transitive dependencies | P1 | S | LOW | — | DONE (added a `pnpm.overrides` field to `package.json` pinning `qs` to `>=6.16.0` and `fast-uri` to `>=3.1.6`, without touching `express` or `@hookform/resolvers`'s own version ranges; `pnpm install` regenerated `pnpm-lock.yaml` resolving `qs@6.16.0` and `fast-uri@4.1.4`; `pnpm audit --prod` went from the 6 findings named in the plan — confirmed identical via a pre-fix re-run — to "No known vulnerabilities found"; `pnpm check` exits 0 (a pre-existing `TS2802` in `analytics.tsx` was present on the unmodified tree too, confirmed via a temporary stash-and-compare, and cleared after `pnpm install` refreshed `node_modules` — unrelated to this plan); `pnpm test` 9/9 pass; `pnpm build` exits 0, `dist/index.cjs` and `dist/public/` produced; manual smoke test against a live `pnpm dev` (own dev server, alternate port 5051, since the owner's dev server already held 5050) submitted the Add Bill form — which routes through `@hookform/resolvers`'s `zodResolver` and thus `ajv`/`fast-uri` — got a "Bill Created" toast and a correct budget-total update, then archived the test bill to leave the DB unchanged; `git status` confirms only `package.json`/`pnpm-lock.yaml` modified; committed on branch `advisor/049-fix-dependency-vulnerabilities`) |
...
| 051  | Wrap `revertPayment` in a transaction | P3 | S | LOW | — | DONE (`server/storage.ts`'s `revertPayment` now runs its 3 selects, 1 update, and 1 conditional delete inside `db.transaction(async (tx) => ...)`, swapping every internal `db.` call for `tx.`, matching the pattern already used by `processAutoPay`/`markPaidAndReset`/`resetPayment`; the auto-pay guard stayed in place unmoved, still throwing before any mutation so its `throw` rolls the transaction back; drift check against commit `e4429a5` showed no changes; `pnpm check` exits 0 and `pnpm test` 9/9 pass (worktree needed a fresh `pnpm install` — `node_modules` was incomplete on checkout, same as plan 048's worktree); the plan's own `grep -n "db\."` verification command has a documentation-only quirk — it flags the now-necessary `db.transaction(` line itself, which isn't a real internal `db.` call, and `grep -c "tx\."` returned 5 as expected; manual test against a live `pnpm dev` + the real Neon DB (own dev server, alternate port 5051, since the owner's dev server already held 5050; worktree's `.env` copied in from the main checkout for the run and removed afterward) confirmed all 3 behavioral checks — a throwaway non-autopay test bill's payment marked paid (creating a next-cycle pending row), then reverted: flipped back to pending and the next-cycle row was deleted, exactly as before; a throwaway autopay test bill's paid payment hit the identical guard error message and 404 status with the paid row provably untouched afterward; reverting a nonexistent payment id still 404s "Payment not found"; both test bills and their payments removed afterward via the DELETE endpoints (bills soft-archive on delete, matching this DB's existing convention from prior test runs, not a new behavior); committed on branch `advisor/051-wrap-revert-payment-in-transaction`) |
